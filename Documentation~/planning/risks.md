...
4. ~~`Append(Action)` sugar?~~ **Decided**: no — `AppendCallback(Action)` stays the only spelling; the overload would ambiguate with `Append(Tween)`/`Append(SequenceBuilder)` composition. Recorded in `Documentation~/guides/conventions.md` API shape rules.
