...
- **Dead-handle late subscriptions — all no-ops** (widened 2026-07-16 from the original OnKill-only decision after the ADR audit found `OnComplete` late-fires identically): `OnComplete`/`OnKill`/`OnStepComplete` on a dead `Tween`/`Sequence` handle do nothing. One uniform rule — a dead handle cannot know whether its record completed or was killed, so firing either callback is a guess. Invariants "OnKill fires only on actual kills" and "OnComplete fires only on actual completion" hold unconditionally; update the existing `TweenBuilderTests` pins for both.
...
- **Global time scale becomes a property** (ADR audit, 2026-07-16, cosmetic): `FT.GlobalTimeScale { get; set; }` (setter throws on negative) replaces the `SetGlobalTimeScale`/`GetGlobalTimeScale` method pair, per C# idiom. The per-phase pair stays methods (`SetTimeScale(phase, scale)` / `GetTimeScale(phase)`) since they take an argument.
- **ADR 0009 × reverse-through-delay composition test**: the new rewind-through-delay behavior must compose with ADR 0009's infinite-loop wrap — specifically for `EveryLoop` delays, where the delay lives inside the cycle slot that the wrap re-enters. Cover forward-wrap, backward-wrap, and reverse-during-initial-delay for both delay types.
...
**Tests**: new/updated coverage for each behavior change above (reverse-through-delay for tween and sequence including the ADR 0009 wrap composition, dead-handle no-op for all three late subscriptions, zero-duration+infinite-loop throw, int rounding golden values, Complete-then-Seek playhead, `CompleteAtCycleEnd`/`CompleteAtCycleStart` on both handles, `SetCapacity(int)`, `FT.GlobalTimeScale` property); full suite green in the compile-check harness and in Unity (Editor + PlayMode).
