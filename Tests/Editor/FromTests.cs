...
namespace PATween.Tests
{
	...
	public class FromTests
	{
		...
		{
			var v = 3f;
			global::PATween.To(() => v, x => v = x, 9f, 1f)
				.From()
				.SetUpdate(UpdatePhase.Manual)
				.Start();
		...
		{
			var v = 42f;
			global::PATween.FromTo(() => v, x => v = x, -1f, 100f, 1f)
				.SetUpdate(UpdatePhase.Manual)
				.Start();
		...
		public void From_GetterRead_AtSnapTime_NotEarlier()
		{
			var v = 1f;
			var calls = 0;
			var builder = global::PATween.From(
				() => { calls++; return v; },
				x => v = x,
				0f,
				1f)
		...
		{
			var v = 1f;
			var builder = global::PATween.From(() => v, x => v = x, 0f, 1f)
				.SetUpdate(UpdatePhase.Manual);
		...
		public void FromTo_NullGetter_Allowed()
		{
			...
			Assert.DoesNotThrow(() =>
			{
				global::PATween.FromTo<float>(null, x => v = x, 0f, 1f, 1f)
					.SetUpdate(UpdatePhase.Manual)
					.Start();
			});
		}
